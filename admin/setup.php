...
/**
 * \file    tab/admin/setup.php
 * \ingroup tab
 * \brief   Tab setup page.
 */
...
require_once '../lib/tab.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "tab@tab"));
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/tab/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'TAB_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'TAB_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print load_fiche_titre($langs->trans($page_name), $linkback, 'object_tab@tab');

// Configuration header
$head = tabAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "tab@tab");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("TabSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'TAB_'.strtoupper($myTmpObjectKey).'_ADDON';
